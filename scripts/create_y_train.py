...
if __name__ == '__main__':
	# load outcome
	METADATA_PATH = '/home/projects/icu/final_gum/jsons_surv/outcome.tsv'
	outcome = pd.read_csv(METADATA_PATH, sep='\t')

	breaks = np.array([0] + [int(x) for x in args.n_window[0].split('-')])  # Make list of integers from e.g ['1-7-14-30-90'] - add 0
	y_train = make_surv_array(outcome.failure_time, outcome.failure, breaks)
